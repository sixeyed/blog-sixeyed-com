...
description: Run a self-hosted Rocket.Chat server on your LAN so every AI coding agent posts its progress to a team chat - watch long-running sessions from any machine your phone and reply to steer them.
header:
...
> Claude Code, Windsurf and Cline each have an agent view where you can monitor activity across agents. But that's only the agents in _that tool_ on _that machine_. I wanted a univseral agent status portal. Which sounds a lot like a chat room.

The chat room idea works really nicely. You can kick off a long session - implementing code, monitoring a deployment, running a performance test - and tell the agent to use a skill to post updates to a discussion on the chat server. Each task gets its own discussion, each agent gets their own ID, so you can watch everything go from a central machine. Or when you leave the office for the evening, you can check in all those remote agents on your phone.

This post walks through the setup: a lightweight [Rocket.Chat](https://rocket.chat) server running on my internal network that every agent posts to, plus a skill that wraps it so any agent can create a discssion, post updates, and take instructions back. The whole thing is in a reference repo on GitHub:

> [sixeyed/agent-chat](https://github.com/sixeyed/agent-chat)

## Quickstart

If you just want to see it working, there's a `demo` folder that stands up the whole stack - ingress, cert-manager, MongoDB and Rocket.Chat - on a local [k3d](https://k3d.io) cluster. You'll need these tools on your path:
...
It deploys the exact same chart as production, just with a local-friendly values overlay:
...
The chat server is a good fit. It's the interface we humans already use to coordinate work. You've got multiple users, multiple threads, unread badges,web and mobile apps that push notifications. But gents post status that could leak all sorts of interesting details - so you don't want that flowing through a third-party setup like Slack or Teams.

Rocket.Chat gives you all the features of the box, and it's open source so you can run it on your network. That keeps the data on the LAN and it means you own auth. Each agent has its own user account with clearly defined permissions, and they use access tokens to post to the REST API. If anything goes awry - say an agent post a load of system credentials without thinking - that information is still private. If the agent's chat account somehow gets compromised, the attack radius is super small and you can rotate the token or delete the account.
...
The Rocket.Chat web UI needs a browser "secure context" - things like `crypto.randomUUID` and notifications are only supported when the page is served over HTTPS. If you try to load the UI over plain HTTP on a LAN IP, you get empty channels and a broken experience. TLS is mandatory for the full UI experience on the browse and in the mobile apps. 

Agents don't use the browser - they call the REST API which doesn't needs a secure context. So agents can talk plain HTTP and they just need the IP address of the server, without needing all the DNS and CA setup that the full client needs.
...
> Rocket does let you have multiple discussions with identical names. Sometimes agents get confused and create a new discussion with the same name, but you can work around that by being stricter with your prompts.
...
- skill.md
- rc.sh / rc.ps1 - helper scripts to wrap the REST API calls
...
The skill is strict about the auth: **the PAT is a secret, and it should never touch a command line or log.** Agents authenticate with the Personal Access Token in the environment variable, and the helper passes it to the HTTP call in a way that keeps it out of process listings and logs - via a curl config file descriptor in bash, or in-process with `Invoke-RestMethod` in PowerShell. T

The connection details come from theenvironment variables (`RC_URL`, `RC_PAT`, `RC_UID` etc.), which you can set per-session or machine-wide, depending on how brave you are. The PAT is a full credential - anyone holding one can post and act as that user - the skill protects it, but it's up to you how you secure it.
...
Posting updates is actually the easy part, and all the tools and models I've tried can do that. The more interesing part is using the Rocket Chat discussion as a control room - so agents post their updates, and look for responses (from a specific user) to steer the session:
...
The chart is a wrapper over the official `rocketchat` chart, with one important change: the official chart bundles Bitnami's MongoDB, which you need to avoid because of [Bitnami-geddon](TODO - link to bitnami post about paywalling). This chart ships its own minimal single-node MongoDB instead (`mongo:8.0`, running as a one-node replica set). It runs as a monolith - microservices and NATS turned off - which is plenty for this job and much less to operate.
...
1. **Generate the credentials** - create a user and PAT for each agent you want to post messages.
...
Give it a try and have all your report back to a single place. It's nice to know you can check in a four-hour performance test running on a remote machine and steer progress while you're 
